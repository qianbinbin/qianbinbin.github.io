...
{{< code lang=c file=csapp-machine-level-representation-of-programs/mstore.c >}}
...
{{< code lang=x86asm file=csapp-machine-level-representation-of-programs/mstore.s >}}
...
这会生成目标代码文件 [mstore.o](/code/csapp-machine-level-representation-of-programs/mstore.o)
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/main.c >}}

生成可执行文件 [prog](/code/csapp-machine-level-representation-of-programs/prog)：
...
{{< code lang=x86asm file=csapp-machine-level-representation-of-programs/mstore.intel.s >}}
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/exchange.c >}}

其[汇编代码](/code/csapp-machine-level-representation-of-programs/exchange.s)为（参数 `xp` 和 `y` 分别保存在 `%rdi` 和 `%rsi` 中）：
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/scale.c >}}

GCC 使用 `-O1` 及以上优化级别时，得到[汇编代码](/code/csapp-machine-level-representation-of-programs/scale.s)：
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/arith.c >}}

GCC 生成[汇编代码](/code/csapp-machine-level-representation-of-programs/arith.s)（指令顺序和书中有区别，不影响结果）：
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/store_uprod.c >}}

C 标准不包括 128 位整数类型，但 GCC 提供了支持，使用 `__int128` 来声明。

其[汇编代码](/code/csapp-machine-level-representation-of-programs/store_uprod.s)：
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/remdiv.c >}}

这里生成的[汇编代码](/code/csapp-machine-level-representation-of-programs/remdiv.s)（与书中有区别，不影响结果）：
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/comp.c >}}

[汇编代码](/code/csapp-machine-level-representation-of-programs/comp.s)：
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/branch.c >}}

我本地生成的[汇编代码](/code/csapp-machine-level-representation-of-programs/branch.s)与书中不同，以书上的为例：
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/absdiff_se.c >}}

[汇编代码](/code/csapp-machine-level-representation-of-programs/absdiff_se.s)：
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/gotodiff_se.c >}}

其中使用了 `goto` 语句，与汇编的实现逻辑相似。事实上如果对它再进行编译的话，产生的[汇编代码](/code/csapp-machine-level-representation-of-programs/gotodiff_se.s)与 `absdiff_se.s` 几乎是一样的。
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/absdiff.c >}}

采用 `-O1` 优化的[汇编代码](/code/csapp-machine-level-representation-of-programs/absdiff.s)为：
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/cmovdiff.c >}}
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/cread.c >}}
...
事实上 GCC 生成的[汇编](/code/csapp-machine-level-representation-of-programs/cread.s)仍然是用条件跳转实现的。
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/fact_do.c >}}

其[汇编代码](/code/csapp-machine-level-representation-of-programs/fact_do.s)为：
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/fact_do_goto.c >}}
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/fact_while.c >}}

采用 `-Og` 编译得到[汇编代码](/code/csapp-machine-level-representation-of-programs/fact_while.s)：
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/fact_while_goto.c >}}
...
使用 `-O1` 优化级别编译 `fact_while.c` 时，生成[汇编代码](/code/csapp-machine-level-representation-of-programs/fact_while.o1.s)：
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/fact_while_gd_goto.c >}}
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/fact_for.c >}}

使用 `-Og` 编译得到[汇编代码](/code/csapp-machine-level-representation-of-programs/fact_for.s)：
...
如果使用 `-O1` 则得到[汇编代码](/code/csapp-machine-level-representation-of-programs/fact_for.o1.s)：
...
{{< code lang=c file=csapp-machine-level-representation-of-programs/switch_eg.c >}}

得到的[汇编代码](/code/csapp-machine-level-representation-of-programs/switch_eg.s)与书中有出入：
...
![](/images/csapp-machine-level-representation-of-programs/general-stack-frame-structure.png)

当过程 P 调用过程 Q 时，会把返回地址压入栈中，这个返回地址也是 P 栈帧的一部分。
