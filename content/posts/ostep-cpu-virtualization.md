...
{{< code lang=c from=16 to=52 file=xv6-public/proc.h >}}
...
{{< code lang=c file=ostep-code/cpu-api/p1.c >}}
...
{{< code lang=c file=ostep-code/cpu-api/p2.c >}}
...
{{< code lang=c file=ostep-code/cpu-api/p3.c >}}
...
{{< code lang=c file=ostep-code/cpu-api/p4.c >}}
...
{{< code lang=c file=ostep-homework/cpu-api/p1.c >}}

2.

{{< code lang=c file=ostep-homework/cpu-api/p2.c >}}

3.

{{< code lang=c file=ostep-homework/cpu-api/p3.c >}}
...
{{< code lang=c file=ostep-homework/cpu-api/p4.c >}}

5. 父进程调用 `wait()` 返回的是子进程的 `pid`，子进程调用 `wait()` 返回值为 `-1`：

{{< code lang=c file=ostep-homework/cpu-api/p5.c >}}

6. `wait(&wstatus)` 等价于 `waitpid(-1, &wstatus, 0)`：

{{< code lang=c file=ostep-homework/cpu-api/p6.c >}}

7. 关闭标准输出后，`printf()` 不会有内容输出：

{{< code lang=c file=ostep-homework/cpu-api/p7.c >}}

8. 使用 `pipe()` 系统调用创建管道，使用 `dup2()` 系统调用将标准输入和输出重定向到管道两端：

{{< code lang=c file=ostep-homework/cpu-api/p8.c >}}
...
{{< code lang=c from=16 to=33 file=xv6-public/proc.h >}}
...
{{< code lang=x86asm file=xv6-public/swtch.S >}}
...
{{< code lang=c file=ostep-homework/cpu-mechanisms/syscall-bench.c >}}
...
{{< code lang=c file=ostep-homework/cpu-mechanisms/context-switch-bench.c >}}
